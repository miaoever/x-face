...
Mat GaborFR::getMagnitude(Mat &real,Mat &imag)
{
	...
	int i,j;
	float*	pFloat = NULL, *pFloatR = NULL, *pFloatI = NULL;
	double*	pDouble = NULL, *pDoubleR = NULL, *pDoubleI = NULL;
...
void GaborFR::gaborFilter(Mat image, int kernelSize, vector<Mat>& GMP) {
    Mat feature, MatTemp1, MatTemp2;
	//GMP.release();
	...
	for(int i=0;i<8;i++)
	{
		for(int j=0;j<5;j++)
		{
			Mat kernelReal= GaborFR::getRealGaborKernel(Size(iSize,iSize),2*CV_PI,i*CV_PI/8 , j,1);
			Mat kernelImage = GaborFR::getImagGaborKernel(Size(iSize,iSize),2*CV_PI,i*CV_PI/8 ,j,1);
			//加了CV_PI/2才和大部分文献的图形一样，不知道为什么！
			Mat outR,outI;
            GaborFR::getFilterRealImagPart(image, kernelReal, kernelImage, outR, outI);
            feature = GaborFR::getMagnitude(outR,outI);
            //feature = feature.t();
            //resize(feature, feature, Size(150,150));
			normalize(feature, feature, 0, 255, CV_MINMAX, CV_8U);
			GMP.push_back(feature);
