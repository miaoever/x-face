...
template <typename _Tp>
void lbp::ELBP_(const Mat& src, Mat& dst, int radius, int neighbors, bool uniform, int* lookup) {
    ...
    if ( uniform ) { // replace content of dst with its resp. lookup value
        for (size_t i=0; i<dst.total(); i++) {
            dst.at<unsigned int>(static_cast<int>(i)) = lookup[ dst.at<unsigned int>(static_cast<int>(i)) ];
        }
    }
}
...
void lbp::ELBP(const Mat& src, Mat& dst, int radius, int neighbors, bool uniform) {
    int lookup[] = {
...
Mat lbp::ELBP(const Mat& src, int radius, int neighbors, bool uniform) { Mat dst; ELBP(src, dst, radius, neighbors, uniform); return dst; }
