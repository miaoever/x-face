...
template <typename _Tp>
void ELBP_(const cv::Mat& src, cv::Mat& dst, int radius = 1, int neighbors = 8, bool uniform = false, int* lookup = NULL);
...
void ELBP(const Mat& src, Mat& dst, int radius = 1, int neighbors = 8, bool uniform = false);
void VARLBP(const Mat& src, Mat& dst, int radius = 1, int neighbors = 8);

// Mat return type functions
Mat OLBP(const Mat& src);
Mat ELBP(const Mat& src, int radius = 1, int neighbors = 8, bool uniform = false);
